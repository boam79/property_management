...
export function QrRegisterForm({ token }: { token: string }) {
  const router = useRouter();
  const bound = registerAssetOnQr.bind(null, token);
  const [state, action, pending] = useActionState(bound, initial);

  useEffect(() => {
    if (state.existingAssetId) {
      router.replace(`/assets/${state.existingAssetId}`);
    }
  }, [state.existingAssetId, router]);
